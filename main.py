...
def changepage1 ():
    ...
    text.see(END)
    while q1+1>b1 :
        findurl('https://disp.cc/b/Beauty?pn='+str(b1)+'&init=0')
        for each in urls :
...
def changepage2 ():
    ...
    b2 = T2_start
    while q2+1>b2 :
        findurl('https://disp.cc/b/Beauty?pn='+str(b2)+'&init=0')
        for each in urls :
            if each in checklist :
                continue
            Download('https://disp.cc/b/'+each)
...
def changepage3 ():
    ...
    b3 = T3_start
    while q3+1>b3 :
        findurl('https://disp.cc/b/Beauty?pn='+str(b3)+'&init=0')
        for each in urls :
            if each in checklist :
                continue
            Download('https://disp.cc/b/'+each)
...
def changepage4 ():
    ...
    b4 = T4_start
    while q4+1>b4 :
        findurl('https://disp.cc/b/Beauty?pn='+str(b4)+'&init=0')
        for each in urls :
            if each in checklist :
                continue
            Download('https://disp.cc/b/'+each)
...
def changepage5 ():
    ...
    b5 = T5_start
    while q5+1>b5 :
        findurl('https://disp.cc/b/Beauty?pn='+str(b5)+'&init=0')
        for each in urls :
            if each in checklist :
                continue
            Download('https://disp.cc/b/'+each)
...
#平均分配開始,結束位置給各個線程
def  distribute () :
    global T1_start,T1_end,T2_start,T2_end,T3_start,T3_end,T4_start,T4_end,T5_start,T5_end
    q = int(e.get())
    # q = 結束
    b = int(w.get())
    # b = 開始
    ...
    T5_start = b+(4*gap)+1
    T5_end = b + (5*gap)
t1 = threading.Thread(target = changepage1)
t2 = threading.Thread(target = changepage2)
t3 = threading.Thread(target = changepage3)
t4 = threading.Thread(target = changepage4)
t5 = threading.Thread(target = changepage5)
root = Tk()
count = IntVar()
count_ = 0
root.geometry('560x200')
root.title('先按路徑選擇,再來設定頁數,最後按開始༼ ∩ ◕ㅂ◕∩ ༽')
...
b = Button(root, text="開始", command=start)
b.grid  (column=0,row=3)
c = Button(root, text="結束", command=close)
